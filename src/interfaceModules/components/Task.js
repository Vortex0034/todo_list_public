...
export default function Task({ id, onChangeDelete }) {
    ...
    function ChangeButton({task}) {
        ...
        return ( 
            <>
                ...
                <AddChangeTaskWindow setVisionFunction={changeWindow} isVision={isWindowShowed} toChange={true} currTask={task} onChangeDelete={onChangeDelete}/>
            </>
        );
    }
    
    function DeleteButton({task}) {
        return <button className="delete-button" id={task.id} onClick={(e) => {deleteTask(task.id);
                                                                               onChangeDelete();}
                                                                            }>Удалить</button>
    }
    ...
    function DoneButton({ task }) {
        ...
        if (!task.done)
            return (
                <button className="done-button" id={task.id} onClick={(e) => {setTaskStatus(true, task);
                                                                              onChangeDelete();
                }}>Отметить выполненным</button>
            );
        else
            return (
                <button className="done-button" id={task.id} onClick={(e) => {setTaskStatus(false, task);
                                                                              onChangeDelete();
                }}>Отметить невыполненным</button>
            );
    }
